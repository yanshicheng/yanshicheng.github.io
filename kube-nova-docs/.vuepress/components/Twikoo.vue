...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if ((this.$frontmatter.comment == undefined || this.$frontmatter.comment) && this.$route.path != '/' && !this.otherPage(this.$route) && !this.isFourZeroFour(this.$route)) {
        setTimeout(() => {
          this.twikooInit()
        }, waitTime)
      }
    ...
    watch: {
      $route(to, from) {
        ...
        if (this.$route.path == '/' || this.otherPage(this.$route) || this.$route.hash != '' || this.isFourZeroFour(to)) {
          return
        }
        ...
        if (from.path == '/' || this.otherPage(from) || !this.getCommentByFrontmatter(from)) {
          this.firstLoad
            ? setTimeout(() => {
                this.twikooInit()
    ...
    methods: {
      twikooInit() {
        twikoo
          .init({
            ...
            envId: 'https://twikoo.yanshicheng.com/',
            el: '#tcomment'
      ...
      // 其他页面
      otherPage(route) {
        if (route.path == '/archives/' || route.path == '/categories/' || route.path == '/tags/') {
          return true
        } else {
          return false
        }
